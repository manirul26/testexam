...
<td><span style='float: left;
  ...
  <div class='close_btn_umzugsgutliste' onClick="opensearch_closediv()">
...
<tr>
<td onclick="storeData({{ $row->pcode}})">
  <p style="padding: 4px;">{{ $row->pcode}}</p>

</td>
<td onclick="storeData({{ $row->pcode}})">  <p style="padding: 4px;">{{ $row->pname}}</p></td>
<td onclick="storeData({{ $row->pcode}})">
  <p style="padding: 4px;">{{ $row->DPPrice}}</p>
